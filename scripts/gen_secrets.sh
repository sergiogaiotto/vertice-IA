...
# ADMIN: 16 chars alfanuméricos (sem / + = pra ser fácil digitar uma vez)
ADMIN_BOOTSTRAP_PASSWORD="$(openssl rand -base64 32 | tr -dc 'A-Za-z0-9' | head -c 16)"
...
──────────────────────────────────────────────────────────────────
  ⚠️  GUARDE essas senhas em um gerenciador (1Password, Bitwarden,
     KeePass). Elas NÃO podem ser recuperadas.

  Próximos passos:
    1) Painel Hostinger → seu projeto → Environment Variables
    2) Cole as 3 variáveis APP_SECRET_KEY / POSTGRES_PASSWORD /
       ADMIN_BOOTSTRAP_PASSWORD
    3) Trigger redeploy

  Login inicial será:
    usuário:  admin
    senha:    a ADMIN_BOOTSTRAP_PASSWORD acima
    (TROQUE imediatamente após o primeiro login pela UI)
